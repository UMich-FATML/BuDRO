...
def run_experiments(name, seed):
    ...
    if name not in ['baseline', 'project', 'SenSR', 'adv_deb', 'proj_bst', 'bst']:
...
#         grid_num = len(depth_grid) *  len(eta_grid) * len(iter_grid)
    ...
    for seed in seeds:
        ...
        for pack in product(*hypers):
            ...
            elif name == 'proj_bst' or name == 'bst':
...
#                     bst = xgb.train(param, dtrain, 10, watchlist, xgb_model = bst)
...
#                     print('Saving results ......')
    ...
    filename = 'result:%d' % seed + '_bst.csv'
