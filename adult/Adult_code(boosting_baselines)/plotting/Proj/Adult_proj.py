...
def get_consistency_bst(X, bst=0, proj = 0, relationship_idx = [33, 34, 35, 36, 37, 38], husband_idx = 33, wife_idx = 38, adv = 0, dataset_orig_test = 0):
    '''
    ...
    print('X_husbands shape:', X_husbands.shape)
    ...
    gender_and_race_consistency = 1.0
...
#     seeds = [58185]
    ...
    if name == 'proj_bst':
        ...
        depth_grid = [3,5,6,8] #4
#         depth_grid = [3] #4
        eta_grid = [0.01, 0.05, 0.1] #3
#         weight_grid = [0.1, 0.5] #3
        weight_grid = [0.1, 0.5, 1] #3
        lambda_grid = [1, 0.5, 0.1, 0.01] #4
#         lambda_grid = [1] #4
...
#             print('X_gender_train:', X_gender_train.shape) 
        ...
        for pack in product(*hypers):
            ...
            if name == 'baseline':
                ...
                for rounds in list(range(50)):
                    ...
                    bst = xgb.train(param, dtrain, 10, watchlist, feval=balance_acc, xgb_model = bst)
    #                 bst = xgb.train(param, dtrain, 10, watchlist, callbacks=[xgb.callback.record_evaluation(eval_result)])
        ...
        #                 print('X_t shape:', X_t.shape)
                        ...
                        gender_race_consistency, spouse_consistency = get_consistency_bst(X_t, bst=bst, adv = 0, dataset_orig_test = dataset_orig_test)
    ...
    filename = 'result:%d' % seed + '_proj.csv'
